...
test.describe("SEO validation", () => {
  ...
  test("publishes crawl and discovery endpoints", async ({ request }) => {
    ...
    expect(sitemapText).toContain("hero-jellyfish-6899910.webp");
  ...
  test("search crawlers receive the guide, FAQ and consistent media without JavaScript", async ({ request }) => {
    for (const bot of ["Googlebot", "bingbot", "Sogou web spider"]) {
      ...
      expect(html).not.toContain("hero-jellyfish-enhanced");
      expect(html).toContain("hero-jellyfish-6899910-4k.mp4");
